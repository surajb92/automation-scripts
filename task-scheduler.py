...
class CalendarPopup(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowFlags(QtCore.Qt.Popup | QtCore.Qt.FramelessWindowHint | QtCore.Qt.WindowStaysOnTopHint) # Change to dialog or tool later to tests
        #self.setFocusPolicy(QtCore.Qt.StrongFocus)
        self.tray_menu_open = False
        self.calendar_focus = False
    '''def focusOutEvent(self, event):
        ...
        if event.reason() == QtCore.Qt.ActiveWindowFocusReason:
            ...
            if self.tray_menu_open:
                ...
                self.calendar_focus = False
                self.hide()
        elif event.reason() in (QtCore.Qt.PopupFocusReason, QtCore.Qt.OtherFocusReason):
            pass
        elif event.reason() == 0 and not self.calendar_focus:
            self.calendar_focus = True
        else:
            self.tray_menu_open = False
            self.calendar_focus = False
            self.hide()
    ...
    def tray_menu(self):
        if not self.tray_menu_open and self.isVisible():
            self.tray_menu_open = True
            print('will this come first?')
...
class SchedulerApp:
    def __init__(self):
        ...
        # Calendar dialog
        self.calendar_popup = CalendarPopup(self.root_widget) #self.calendar)
        ...
        # Focus handler
        self.app.focusObjectChanged.connect(lambda old=None,new=None: self.focus_handler(old,new)) # This lambda solution is as amazing as ever lol
        ...
        self.task_format.setBackground(QtGui.QColor("lightgreen"))
    ...
    def task_manager(self,date):
        ...
        self.task_popup = QtWidgets.QWidget()
    ...
    def handle_tray_click(self, reason):
        if reason == QtWidgets.QSystemTrayIcon.Trigger : # i.e. if left-clicked
            ...
            if self.calendar_popup.isVisible():
                ...
                tray_pos = QtGui.QCursor.pos() # 
                tray_pos = QtCore.QPoint(QtWidgets.QWidget.mapToGlobal(1900, 800))
                self.calendar_popup.move(tray_pos)
                self.calendar_popup.show()
                #self.calendar_popup.raise_()
                #self.calendar_popup.setFocus()
                #self.calendar_popup.calendar.setFocus()
                print(f"Popup shown at {self.calendar_popup.x()},{self.calendar_popup.y()}")
        else:
            print("do we get here for right clicks?")
                
    def focus_handler(self,old,new):
        # print("Focus: ",old," --> ",new) 
        if (new == None):
            if self.calendar_popup.tray_menu_open:
                self.calendar_popup.tray_menu_open = False
            else:
                pass
                #self.calendar_popup.hide()
        #if old == self.calendar_popup:
